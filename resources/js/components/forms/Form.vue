...
const formFields: Record<string, ValueType | ValueType[]> = props.specs
  ? Object.fromEntries(Object.entries(props.specs).map(
    ([k, v]) => [k, v.element === 'select' ? v.selected : v.value])
  )
...
watch(form, (data) => Object.entries(data).forEach(([field, value]) => {
...
watch(() => props.specs, (newSpecs, old) => {
    Object.entries(newSpecs!).forEach(([field, spec]) => {
      const newValue = spec.element === 'select' ? spec.selected : spec.value;
      const oldValue = old?.[field]?.element === 'select' ? old[field].selected : old?.[field]?.value;
                ...
                <div class="@container/form grid w-full grid-cols-[auto_1fr] gap-y-4">
                    <slot :form="form">
                        <template v-for="({ element, ...def }, name) of specs" :key="name">
                            <Component :is="name2component(element)" :name="<keyof typeof formFields>name"
                                v-bind="{ ...def }" v-model="form[name] as ValueType" :error="form.errors[name]"
