...
<template>
  <Card :resource="family" entity="family" type="show">
    <CardHeader :title="family.name" :kicker="_('Family')" avatar="lg" />

    <CardContent>
      <ContentGrid min-width="35cqw">
        <ModalLink v-for="member in family.members" :key="member.id"
          ...
          :href="route('members.show', member.id)" :title="member.name" prefetch="click">
