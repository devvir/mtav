...
import { Monitor, Moon, Sun, Droplet, Trees, Sunset as SunsetIcon, Circle, Contrast, Sparkles } from 'lucide-vue-next';
...
const themes = [
  ...
  },
  {
    ...
    colors: { primary: 'hsl(185, 80%, 42%)', surface: 'hsl(200 25% 98%)', text: 'hsl(200 20% 12%)' }
  },
  {
    value: 'forest',
    Icon: Trees,
    name: 'Forest',
    desc: 'Earthy greens',
    colors: { primary: 'hsl(145, 65%, 40%)', surface: 'hsl(140 15% 98%)', text: 'hsl(140 25% 10%)' }
  },
  {
    value: 'sunset',
    Icon: SunsetIcon,
...
<template>
  <div class="space-y-8">
    ...
    <div>
      ...
      <div class="inline-flex gap-1 rounded-lg bg-surface-sunken p-1 border border-border-subtle">
        <button
          v-for="{ value, Icon, label } in modes"
          :key="value"
          @click="updateMode(value)"
          :class="[
            'flex min-h-[44px] @md:min-h-[36px] items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition-all focus:outline-0 focus:ring-2 focus:ring-focus-ring focus:ring-offset-2',
            mode === value
    ...
    <div>
      ...
      <div class="grid gap-3 @md:grid-cols-2 @xl:grid-cols-3">
        <button
          ...
          :class="[
            'group relative flex min-h-[44px] items-center gap-3 rounded-xl border-2 p-3 text-left transition-all focus:outline-0 focus:ring-2 focus:ring-focus-ring focus:ring-offset-2',
            colorTheme === theme.value
              ? 'border-interactive shadow-md'
              : 'border-border bg-surface hover:border-border-strong hover:shadow-sm',
          ]"
          :style="colorTheme === theme.value ? { backgroundColor: theme.colors.primary + '08' } : {}"
        >
          <!-- Theme Icon/Preview -->
          <div
          ...
          <div class="flex-1 min-w-0">
            <div class="text-sm font-semibold text-text truncate" :title="_(theme.name)">{{ _(theme.name) }}</div>
            <div class="text-xs text-text-subtle truncate" :title="_(theme.desc)">{{ _(theme.desc) }}</div>
