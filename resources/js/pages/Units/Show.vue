...
<template>
  ...
  <Breadcrumbs>
    ...
    <Breadcrumb route="units.show" :params="unit.id"
      >{{ _('Unit') }} {{ unit.identifier }}</Breadcrumb
    >
  </Breadcrumbs>
      ...
      <Card class="size-full">
            ...
            <div class="flex min-w-0 flex-1 items-center gap-4">
              ...
              <div class="min-w-0 flex-1">
                <h2 class="truncate text-2xl font-semibold text-text">
                  {{ _('Unit') }} {{ unit.identifier }}
                </h2>
                <p
                  v-if="unit.type && 'name' in unit.type"
                  class="mt-1 truncate text-sm text-text-subtle"
                >
                  {{ unit.type.name }}
                </p>
              </div>
        ...
        <div class="mt-6 space-y-6 px-3 py-5">
          ...
          <div class="grid gap-4 @md:grid-cols-2">
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 overflow-hidden">
                  ...
                  <ModalLink
                    ...
                    class="truncate text-lg font-semibold text-text-link hover:text-text-link-hover focus:ring-2 focus:ring-focus-ring focus:ring-offset-1 focus:outline-none"
                  >
                    {{ unit.type.name }}
                  </ModalLink>
                </div>
            ...
            <!-- Family -->
            <div
              v-if="unit.family && 'name' in unit.family"
              class="rounded-lg border border-border bg-surface-sunken p-4"
            >
              <div class="flex items-center gap-3">
                <div class="rounded-md bg-interactive/10 p-2">
          ...
          <!-- Project Info -->
          <div
            v-if="unit.project && 'name' in unit.project"
            class="rounded-lg border border-border bg-surface-sunken p-4"
          >
