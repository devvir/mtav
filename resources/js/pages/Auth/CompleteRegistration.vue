...
const props = defineProps<{
  ...
  family: ApiResource<Family> | null;
}>();
...
const submit = () =>
  form.post(route('invitation.update'), {
...
const familyProject =
  props.family && hasProjectData(props.family.project) ? props.family.project : undefined;
const project =
  familyProject ||
  (props.user.projects && props.user.projects.length > 0 ? props.user.projects[0] : undefined);
</script>
    ...
    <div class="mb-6 space-y-4 md:mb-8 md:space-y-6">
      ...
      <h1 class="text-center text-xl font-semibold md:text-2xl lg:text-3xl">
        <span class="inline-block"
          >{{ _('Welcome') }}, {{ props.user.firstname || props.user.name.split(' ')[0] }}!</span
        >
        <br class="md:hidden" />
        <span
          class="inline-block text-base font-normal md:ml-2 md:text-2xl md:font-semibold lg:text-3xl"
      ...
      <div class="flex items-center justify-center">
        <div
          class="w-full rounded-lg bg-purple-100 px-4 py-3 text-center text-sm font-medium text-purple-700 md:text-base dark:bg-purple-900/30 dark:text-purple-300"
        >
      ...
      <div class="rounded-lg border bg-card p-4">
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div v-if="iAmAdmin && props.user.projects?.length" class="space-y-1 text-center">
            ...
            <div class="text-sm font-medium text-text">
              {{ props.user.projects.map((p: Project) => p.name).join(', ') }}
            </div>
          </div>
