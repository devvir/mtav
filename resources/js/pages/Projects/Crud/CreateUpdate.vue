...
const props = defineProps<{
  ...
  project?: ApiResource<Project>; // Edit-only
}>();

const adminOptions: SelectOptions = {};
props.admins.forEach(
  (admin: Admin) => (adminOptions[admin.id] = `${admin.firstname} ${admin.lastname}`),
);
...
const formSpecs: FormSpecs = {
  ...
  admins: {
    ...
    selected: props.project?.admins?.map((a: Admin) => a.id) ?? [],
