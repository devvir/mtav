...
* **Browser Testing:** Use Pest Browser (Playwright-based, PHP tests in tests/Browser/) not Cypress. Wait for `.animate-pulse` CSS class to disappear instead of artificial delays - the class is guaranteed to only be present while deferred data is loading
* **State Management:** Use Inertia props as source of truth. Use `useLocalState()` for session-only state. No Pinia - it's unused.