...
This document provides architectural patterns and key principles for authorization in the MTAV system.

## Key Principles

### Superadmin Bypass

**Critical:** Superadmins bypass ALL policies via `Gate::before()` in `AppServiceProvider.php`:
...
**Impact:**
- Superadmins can perform ANY action on ANY resource
- Policies only need to handle guest/member/admin authorization
- Never implement superadmin checks in individual policies

### Policy Architecture

**Location:** `app/Policies/`
**Naming:** Laravel standard - `{Model}Policy.php`
**Discovery:** Automatic via naming convention
...
1. **Guests** - Usually denied or very limited access
2. **Members** - Project-scoped access, can view/modify own data
3. **Admins** - Can manage resources within their projects

**Common Patterns:**
- `return true;` - Allow anyone (including guests)
- `return $user->isAdmin();` - Admin-only actions
- `return $user->is($resource);` - Own resource only
- Project-scoped checks for admin actions

### Optimization: FormRequest Authorization

**Pattern:** Some policies return `true` but full authorization happens in FormRequest classes.

**Why:** Avoids N+1 queries when loading collections with policy checks.

**Location:** `app/Http/Requests/`

**When to Check:** Look for policies that seem "too permissive" - the real constraint is likely in the corresponding FormRequest.

## Quick Reference

To see current authorization for any model:
1. Check `app/Policies/{Model}Policy.php` for basic constraints
2. Check `app/Http/Requests/*{Model}*Request.php` for full validation
3. Remember: superadmins bypass everything via Gate

For specific authorization logic, always check the source files rather than relying on this documentation to avoid outdated information.
...
- All mutations require admin role only
...
| Show    | UnitTypePolicy::view()    | Anyone        |
| Create  | UnitTypePolicy::create()  | Only Admins   |
| Update  | UnitTypePolicy::update()  | Only Admins   |
| Delete  | UnitTypePolicy::delete()  | Only Admins   |
| Restore | UnitTypePolicy::restore() | Only Admins   |
